module.exports = (io) => {
  ...
  router.get('/:symbolFrom/:symbolTo/:window?/:timing?', (req, res, next) => {
    ...
    collectInfo(symbolFrom, symbolTo, startTime, () => {
      mysqlConnection.executeQueries([{
        ...
        {
          sql: "select * from orders where symbolFrom=? and time>=? order by time asc",
        ...
        binanceModule.getAllOpenOrder({}, (bRes) => {
          ...
          binanceModule.getCandleHistory((symbolFrom + symbolTo).toUpperCase(), timing, +startTime, +endTime, (history) => {
            ...
            analytics.stopPriceCalc(historyStore);
    ...
    binanceModule.setSellStopLimit({
      ...
      if (response.status != 200) {
        ...
        mysqlConnection.executeQueries({
          ...
          params : [req.body.symbol, req.body.type, response.data.orderId, parseFloat(req.body.limitPrice)]
        }, true).then((resp) => {
          res.json(response.data);
        }).catch((error) => {
          logger.error('Query error insert stop limit', error);
          res.json({msg : 'Query error insert stop limit'}, 400);
        });
      }
    });
  });
  ...
  function collectInfo(symbolFrom, symbolTo, startTime, callback) {
